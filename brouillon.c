...
int projet_sign_and_verify(uint32_t* witness)
{
    ...
    for (int i = 0; i < 1; i++) {
        ...
        uint8_t* sigBytes = malloc(1500);
        size_t sigByteLength = 1500;
        ret = serializeSignature2(sig, sigBytes, sigByteLength, params);
        printf("%d", ret);
    }
    clock_t end = clock();
    double time_spent = (double)(end - begin) / CLOCKS_PER_SEC;

    printf("Temps pour 1000 exécutions: %f\n", time_spent);
